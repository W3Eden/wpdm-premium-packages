...
global $current_user, $payment_methods;
...
<div id="select-payment-method">
    <form action="" name="payment_form" id="payment_form" method="post">
        ...
        <div class="panel panel-default text-right" id="selected-payment-gateway-action">
            <?php if(get_option('__wpdm_checkout_privacy', 0) == 1){ ?>
                <div class="panel-heading text-left">
                    <label><input style="margin: 0 5px 0 0 !important;" class="wpdm-checkbox" id="checkout-terms-agree" type="checkbox" required="required" value="1" name="__iagree">   <?php echo get_option('__wpdm_checkout_privacy_label'); ?></label> ( <a href="<?php echo get_permalink(get_option('wp_page_for_privacy_policy')); ?>" target="_blank">Read <?php echo get_the_title(get_option('wp_page_for_privacy_policy')); ?></a> )
                </div>
            <?php } ?>
            ...
            <?php } else { ?>
                <div class="alert alert-danger"><?php _e('No payment option is available!', 'wpdm-premium-packages'); ?></div>
            <?php } ?>
        </div>
