jQuery(function ($) {
    ...
    if($('#payment_form input[name="payment_method"]:checked').val() != undefined){
        ...
        $.post(wpdm_url.ajax, {action: 'set_payment_method_for_order', method: $('#payment_form input[name="payment_method"]:checked').val()}, function (res) {
    ...
    $body.on('change', '#payment_form input[name="payment_method"]', function () {
        ...
        $.post(wpdm_url.ajax, {action: 'set_payment_method_for_order', method: $(this).val()}, function (res) {
            if(res.button === 'custom'){
                $('#checkout-terms-agree').prop('checked', true).prop('disabled', true);
                $('#pay_btn').hide();
                $('#wpdmpp-custom-payment-button').html(res.html).show();
...
function getkey(file, order_id, btn_id){
    ...
    jQuery.post(wpdm_url.home,{execute:'getlicensekey',fileid:file,orderid:order_id},function(_res){
        ...
        res = WPDM.el("div", {'class': 'input-group'}, res + WPDM.el("div", {'class': 'input-group-append'}, WPDM.el("button", {type: 'button', 'class': 'btn btn-secondary', id: 'btn-copy-key', onclick: "WPDM.copy('lkcont')" }, WPDM.el("i", {'class': 'fas fa-copy'})+' Copy')));
