...
/**
 * @return \WPDMPP\WPDMPremiumPackage
 */
function WPDMPP()
{
    global $wpdmpp;
...
function wpdmpp_delete_product()
{
    if (is_user_logged_in() && isset($_GET['dproduct'])) {
        ...
        if ($current_user->ID == $pro->post_author) {
            ...
            \WPDM\__\Session::set('dpmsg', __('Product Deleted', 'wpdm-premium-packages'));
            header("location: " . $_SERVER['HTTP_REFERER']);
            die();
        }
    }
...
function wpdmpp_update_guest_billing()
{
    ...
    \WPDMPP\Libs\Order::Update(array('billing_info' => serialize($billinginfo)), \WPDM\__\Session::get('guest_order'));
    die('Saved!');
}
...
{
    return \WPDM\__\Session::get('orderid');
}
...
function wpdmpp_validate_download($package)
{
    ...
    if (floatval($price) > 0) {
        ...
        if ((int)\WPDM\__\Session::get('__wpdmpp_authorized_download') === 1) return $package;
