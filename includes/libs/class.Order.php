...
class Order
{
    ...
    function newOrder($id, $title, $items, $total, $userid, $order_status = 'Processing', $payment_status = 'Processing', $cart_data = '', $order_notes = "", $payment_method = "")
    {
        ...
        $ip = __::get_client_ip();
        $ret = $wpdb->insert("{$wpdb->prefix}ahm_orders", array('order_id' => $id, 'title' => $title, 'date' => time(), 'items' => $items, 'total' => $total, 'order_status' => $order_status, 'payment_status' => $payment_status, 'cart_data' => $cart_data, 'uid' => (int)$userid, 'order_notes' => $order_notes, 'payment_method' => $payment_method, 'download' => 0, 'IP' => $ip, 'currency' => $currency, 'auto_renew' => $auto_renew));
    ...
    static function add_note($id, $data, $type = 'messages')
    {
        ...
        if (isset($data['admin']) && $send_email) {
            ...
            $message = __MailUI::panel("Note:", array(wpautop($data['note'])), $viewlink_admin);
        ...
        if (isset($data['customer']) && $send_email) {
            ...
            $message = __MailUI::panel("Note:", array(wpautop($data['note'])), $viewlink_customer);
    ...
    static function complete_order($id, $email_notify = true, $payment_method = null)
    {
        ...
        // to buyer
        if (!$userid)
            \WPDM\__\Email::send("purchase-confirmation-guest", $params);
        else
    ...
    static function renewOrder($id, $sub_id, $email_notify = true, $payment_method = null, $date = null)
    {
        ...
        // to buyer
        \WPDM\__\Email::send("renew-confirmation", $params);
