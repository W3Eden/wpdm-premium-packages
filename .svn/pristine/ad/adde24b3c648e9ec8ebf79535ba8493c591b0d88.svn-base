<?php
/**
 * Order details Template for [wpdmpp_purchases] shortcode
 *
...
$o      = $order;
$order  = $order->GetOrder(sanitize_text_field($_GET['id']));
$link   = get_permalink();
                ...
                <span class="pull-right">
                        <a class="btn btn-info btn-xs white btn-invoice" href="#"
                           onclick="window.open('?id=<?php echo $order->order_id; ?>&amp;wpdminvoice=1','<?php _e('Invoice', 'wpdm-premium-packages'); ?>','height=720, width = 750, toolbar=0'); return false;">
                            <i class="fas fa-file-invoice"></i> <?php _e('Invoice', 'wpdm-premium-packages'); ?>
                        </a>
            ...
            <table class="table" style="margin:0;border:0;">
                ...
                foreach ($items as $item) {
                    ...
                    if ($order->order_status == 'Completed') {
                        if ((int)get_post_meta($item['pid'], '__wpdm_enable_license_key', true) === 1) {
