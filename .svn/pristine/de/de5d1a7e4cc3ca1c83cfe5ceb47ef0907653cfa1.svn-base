...
<div class="panel-body" id="csp">
    <div class="list-group payment-gateway-list cols-2">
        <?php
        ...
        foreach ($payment_methods as $payment_method) {
            ...
            if ( class_exists( $payment_gateway_class ) ) {
                if ( isset( $settings[$payment_method]['enabled']) && $settings[$payment_method]['enabled'] == 1) {
                    ...
                    echo '<label class="list-group-item '.$pg_item_class.'"><input class="wpdm-radio wpdm-payment-method" type="radio" name="payment_method" value="' . $payment_method . '" > ' . $name . '</label>';
                }
            }
        }
        ?>
