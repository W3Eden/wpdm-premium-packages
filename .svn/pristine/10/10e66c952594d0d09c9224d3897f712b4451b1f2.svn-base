    ...
    foreach ($payment_methods as $payment_method) {
        ...
        if ( class_exists( $payment_gateway_class ) ) {
            if ( isset( $settings[$payment_method]['enabled']) && $settings[$payment_method]['enabled'] == 1) {
                ...
                echo '<label class="list-group-item '.$pg_item_class.'" id="'.$row_id.'"><input class="mr-2" type="radio" name="payment_method" '.checked($payment_methods[0], $payment_method, false).' value="' . $payment_method . '" ><span class="grateway-name">' . $name . '</span></label>';
            }
        }
    }
    ?>
