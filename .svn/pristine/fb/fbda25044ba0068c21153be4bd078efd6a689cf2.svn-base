...
function wpdmpp_get_purchased_items(){
    ...
    if(is_user_logged_in())
        echo json_encode(\WPDMPP\Libs\Order::getPurchasedItems());
    else
...
{
    $wpdmpp_txt = array(
        'pay_now' => __("Pay Now","wpdm-premium-package"),
    );
...
function wpdmpp_expiry_check(){
    ...
    foreach ($orders as $_order){
        $expire_date = $_order->expire_date > 0?$_order->expire_date:$_order->date + (get_wpdmpp_option('order_validity_period', 0)*86400);
...
function wpdmpp_send_order_expiration_email(){
    if(isset($_REQUEST['wpdmppsoee']) && $_REQUEST['wpdmppsoee'] == 1){
