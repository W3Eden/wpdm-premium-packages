            ...
            if (wpdm_query_var('__jsvalidate', 'int') === 1) {
                ...
                if ($payment->state === 'approved') {
                    ...
                    if ($payment_amount >= $cart_total) {
                        ...
                        $billing_info = array(
                            'first_name' => $first_name ? $first_name : sanitize_text_field($_REQUEST['payer']['payer_info']['first_name']),
                            'last_name' => $last_name ? $last_name : sanitize_text_field($_REQUEST['payer']['payer_info']['last_name']),
                            'order_email' => $order_email ? $order_email : sanitize_email($_REQUEST['payer']['payer_info']['email']),
                            'email' => $order_email ? $order_email : sanitize_email($_REQUEST['payer']['payer_info']['email'])
                        );
                            ...
                            onAuthorize: function (data, actions) {
                                return actions.payment.execute()
                                    .then(function (response) {
                                        ...
                                        $.post('<?php echo $this->NotifyUrl; ?>&__jsvalidate=1'+'&email='+$('#email_m').val()+'&name='+$('#f-name').val(), response, function (res) {
                                            if (res.success == true) {
                    ...
                    loadScripts(['https://www.paypal.com/sdk/js?client-id=<?php echo ($env === 'sandbox') ? $this->client_id_sandbox : $this->client_id; ?>&vault=true']).then(() => {
                        paypal.Buttons({
                            ...
                            onApprove: function (data, actions) {
                                ...
                                $('#paymentform').append('<div class="alert alert-success">'+wpdm_asset.spinner+' <?php _e('Completing Order...', 'wpdm-premium-packages'); ?></div>');
                                $.post('<?php echo $this->NotifyUrl ?>&__jsvalidate=1', {subscriptionID: data.subscriptionID, name: $('#f-name').val(), email: $('#email_m').val()}, function (res) {
                                    if (res.success == true) {
                                        location.href = res.redirect;
                                    }
                                    else {
