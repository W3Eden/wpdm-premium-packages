...
$order_notes    = '';
$orderObj          = new \WPDMPP\Libs\Order(wpdm_query_var('id'));
...
$link           = get_permalink();
$o              = $orderObj;
$order          = $orderObj->getOrder(wpdm_query_var('id'));
$extbtns        = "";
$extbtns        = apply_filters("wpdmpp_order_details_frontend", $extbtns, $order);

//Check order status
if($order->expire_date == 0 && get_wpdmpp_option('order_validity_period', 0) > 0) {
...
if( $order->uid == $current_user->ID ) {
    ...
    <div class="card card-default card-purchases dashboard-card">
        ...
        <div class="card-body-table">
            <table class="table wpdm-table-clean m-0">
                <thead>
                <tr>
            ...
            <div class="card-footer  border-0">
                ...
                <div class='pull-right'>
                    ...
                    <div class="pull-right" style="margin-left:10px" id="proceed_<?php echo $order->order_id; ?>">
                        <a class='btn btn-success white btn-xs' onclick="return proceed2payment_<?php echo $order->order_id; ?>(this)" href="#"><b><?php echo $pnow; ?></b></a>
                    </div>
                </div>
                <div class="clear"></div>
            </div>
