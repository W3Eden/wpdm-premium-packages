...
function wpdmpp_show_cart(){
    ...
    return $T->fetch('checkout-cart/cart.php', WPDMPP_TPL_DIR);
}
...
function wpdmpp_load_saved_cart(){
    if( isset( $_REQUEST['savedcart'] ) ) {
        ...
        $saved_cart_data = \WPDM\libs\Crypt::Decrypt($saved_cart_data);
...
function wpdmpp_waytocart($post, $btnclass = 'btn-info'){
    ...
    */
    ...
    if( $price_variation ) {
        ...
        $html = "<form method=\"post\" action=\"\" name=\"cart_form\" class=\"wpdm_cart_form\" id=\"wpdm_cart_form_{$post['ID']}\">
                    <input type=\"hidden\" name=\"addtocart\" value=\"{$post['ID']}\">";

        $html .= '<button class="btn '.$btnclass.'  btn-addtocart" type="submit" ><i class="fa fa-shopping-cart icon-white"></i> '.__("Add to Cart","wpdm-premium-packages").'<span class="price-'.$post['ID'].' label label-price">'.$currency_sign.wpdmpp_effective_price($post['ID']).'</span></button></form>';
    }
