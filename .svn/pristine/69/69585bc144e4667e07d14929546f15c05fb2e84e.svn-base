	...
	class WPDMPremiumPackage
	{
		...
		private function init_hooks()
		{
			...
			*/
			...
			add_filter( 'wpdm_frontend', array( $this, 'frontendTabs' ) );
		...
		{
			global $post;
			include \WPDM\__\Template::locate('checkout-cart/checkout.php', dirname(__FILE__) . '/templates'.WPDM()->bsversion."/", WPDM_TPL_FALLBACK);
			die();
		}
		    ...
		    if(wpdm_query_var('wpdmppd') !== '' || wpdm_query_var('wpdmppdl') !== '') {
                ...
                if (@in_array($PID, $items) && $OID != '' && is_user_logged_in() && $current_user->ID == $odata->uid && $odata->order_status == 'Completed') {
                    ...
                    if (count($cfiles) > 0 && !isset($cfiles[wpdm_query_var('ind')])) {
                        if (count($cfiles) > 1) {
                            $zipped = \WPDM\__\FileSystem::zipFiles($cfiles, $package['post_title'] . " " . $odata->oid);
			...
			if (Session::get('orderid')) {
                ...
                if (is_object($order_info) && $order_info->order_id) {
                    ...
                    if($order_info->order_status !== 'Completed') {
                        ...
                        $order->updateOrderItems(wpdmpp_get_cart_data(), $order_info->order_id);
		...
		function paynow()
		{
			if (isset($_REQUEST['task']) && $_REQUEST['task'] == "paynow") {
				...
				$data = array(
					'payment_method' => wpdm_query_var('payment_method', 'txt'),
		...
		function place_order($order_id)
		{
            ...
            global $wpdb;
			$order = new Order();
			$order_total = $order->calcOrderTotal($order_id);
			$tax = 0;
            ...
            $item = $items[0];
            $product_name = get_the_title(wpdm_valueof($item, 'pid'));
            $order_title = str_replace(array("{{ORDER_ID}}", "{{PRODUCT_NAME}}"), array($order_id, $product_name), get_wpdmpp_option('order_title'));

            $cart_id = wpdmpp_cart_id();
            $coupon = get_option($cart_id."_coupon", array('code' => '', 'discount' => 0));
            $subtotal = wpdmpp_get_cart_subtotal();
            if($subtotal > 0)
                $subtotal = $order_total - $coupon['discount'];
            if(wpdmpp_tax_active() && Session::get('tax') && $subtotal > 0){
                $tax = Session::get('tax');
