...
if (!class_exists('WPDMPremiumPackage')):
	...
	class WPDMPremiumPackage
	{
		...
		function wpdmpp_meta_box_sales_overview()
		{
            ...
            ob_start();
			include \WPDM\Template::locate('metaboxes/product-sales-overview.php', WPDMPP_TPL_DIR);
            $data = ob_get_clean();
		...
		{
			global $post;
			include \WPDM\Template::locate('checkout-cart/checkout.php', WPDMPP_TPL_DIR);
			die();
		}
		...
		function wpdmpp_purchased_items($params = array()){
			...
			if(isset($params[2]) && $params[1] == 'order')
			    include wpdm_tpl_path('user-dashboard/order-details.php', WPDMPP_TPL_DIR);
			else //if(isset($params[1]) && $params[1] == 'orders')
			    include wpdm_tpl_path('user-dashboard/purchase-orders.php', WPDMPP_TPL_DIR);
			//else
		...
		function download()
		{
		    ...
		    if(wpdm_query_var('wpdmppd') !== '') {

		        $wpdmdd = Crypt::decrypt(wpdm_query_var('wpdmppd'));
                ...
                if($domain !== '' && $domain === wpdm_query_var('domain')){
                    if(!user_can($odata->uid, 'manage_options')) {
                        ...
                        Session::get('guest_order', $OID, 18000);
                    }
                }

                $expire_date = $odata->expire_date > 0 ? $odata->expire_date : ($odata->date + (get_wpdmpp_option('order_validity_period', 0) * 86400));
        ...
        function set_payment_method(){
            ...
            if(wpdm_query_var('method', 'txt') != ''){
                ...
                if(method_exists($payment->Processor, 'customPayButton')){
                    ...
                    if($cb != '')
                        wp_send_json(array('button' => 'custom', 'html' => $cb));
                }
                wp_send_json(array('button' => 'default', 'html' => ''));
            }
        }
        ...
        function fetch_template_tag($vars)
        {
            ...
            if ($effective_price > 0 || get_post_meta($vars['ID'], '__wpdm_pay_as_you_want', true) == 1) {
                ...
                $vars['addtocart_url'] = wpdmpp_cart_page("addtocart={$vars['ID']}");
