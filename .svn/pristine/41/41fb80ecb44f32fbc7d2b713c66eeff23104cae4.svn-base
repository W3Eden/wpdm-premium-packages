...
<?php $content = ob_get_clean(); ?>
    ...
    <div class="w3eden admin-orders">
        ...
        $menus = [
            ...
            ['link' => "edit.php?post_type=wpdmpro&page=orders&task=createorder", "name" => __("Create New", "download-manager"), "active" => false],
            ['link' => "edit.php?post_type=wpdmpro&page=orders&task=vieworder&id=".wpdm_query_var('id', 'txt'), "name" => __("Order #".wpdm_query_var('id', 'txt'), "download-manager"), "active" => true],
        ];



        WPDM()->admin->pageHeader(esc_attr__( "View Order", "wpdm-premium-packages" ), 'cart-arrow-down color-purple', $menus);
        ?>
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <div class="pull-right" style="margin-top: -3px">
                        <button type="button" class="btn btn-info btn-xs"  data-toggle="modal" data-target="#changetrannid">
                            <?= esc_attr__( 'Edit', PM_TEXT_DOMAIN ) ?>
                        </button>
                    </div>
                    <?php _e("Order ID", "wpdm-premium-packages");  ?>
                </div>
                <div class="panel-body">
                    <span class="lead"><strong><?php echo apply_filters("wpdmpp_admin_order_details_order_id", $order->order_id, $payment_method); ?></strong> <?php if($order->trans_id) { echo  "<span title='".sprintf(__("%s transaction ID", "wpdm-premium-packages"), $payment_method )."' style='font-size: 9pt' class='text-muted ttip' id='tnid'>( ". apply_filters("wpdmpp_admin_order_details_trans_id", $order->trans_id, $payment_method) . " )</span>"; } ?></span>
                        ...
                        <div class="modal-dialog" role="document" style="width: 350px">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <input type="text" placeholder="<?php echo __("New Transection ID", "wpdm-premium-packages"); ?>" value="<?= $order->trans_id ?>" class="form-control input-lg" id="changetid" />
                                </div>
...
<script>
    jQuery(function($){
        ...
        $('#change_transection_id').on('click', function(){
            ...
            $.post(ajaxurl,{action:'wpdmpp_change_transection_id', order_id:'<?php echo $oid; ?>', trans_id:$('#changetid').val(), __nonce:'<?php echo wp_create_nonce(NONCE_KEY);?>'},function(res){
                $('#tnid').html("( "+$('#changetid').val()+" )");
                WPDM.unblockUI('#changetrannid .modal-content');
                alert(res);
                $('#changetrannid').modal('hide');
            }).fail(function (res){
                alert('<?= esc_attr__( 'Action Failed!', PM_TEXT_DOMAIN ) ?>');
                WPDM.unblockUI('#changetrannid .modal-content');
            });
        });
