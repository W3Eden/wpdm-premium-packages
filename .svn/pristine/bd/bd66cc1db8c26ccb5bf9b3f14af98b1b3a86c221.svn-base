...
class Payment{
    ...
    static function GateWaySettings($gateway){
        ...
        return wpdm_option_page($options)."<script>jQuery(function($){ $('#enable_".$class_shortname."').on('click', function(){ if(this.checked) $('#pmstatus_".$class_shortname."').html('<span class=\"color-green\">Active <i class=\"far fa-check-square\"></i></span>'); else $('#pmstatus_".$class_shortname."').html('<span class=\"color-red\">Inactive  <i class=\"far fa-minus-square\"></i></span>');  }); });</script>";
    }
