...
LIC;
                        ...
                        if (count($files) > 1 && $order->order_status == 'Completed') {
                            ...
                            foreach ($files as $ind => $ff) {
                                ...
                                $index  = $ind;
                                $ff     = "<a class='list-group-item' style='padding:10px 15px;' href='{$download_link}&ind={$index}'>" . $title . " <span class='pull-right'><i class='fa fa-arrow-alt-circle-down'></i></span></a>";
                                $indf   .= "$ff";
                            }
                        }
...
<a class="btn btn-xs btn-success btn-group-item" href="#" data-toggle="modal" data-target="#dpop" onclick="jQuery('#dpop .modal-body').html(jQuery('#indvd-{$ditem->ID}').html());"><i class="fa fa-list"></i></a></div><div  id="indvd-{$ditem->ID}" style="display:none;"><div class='list-group'>{$indf}</div>
SPEC;
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title m-0"><?php _e('Download Single File','wpdm-premium-packages'); ?></h5>
